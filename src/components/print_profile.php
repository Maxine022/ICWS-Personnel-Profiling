...
<section>
  ...
  <table>
    ...
    <tr><td><strong>Birthdate:</strong></td>
    <td>
      <?php
        $birthdate = $employee['birthdate'] ?? '';
        if ($birthdate && $birthdate !== 'N/A') {
          $dt = DateTime::createFromFormat('Y-m-d', $birthdate);
          echo $dt ? $dt->format('m-d-Y') : htmlspecialchars($birthdate);
        } else {
          echo 'N/A';
        }
      ?>
        ...
        <td>
          <?php
            if (!empty($record['start_date'])) {
              $dt = DateTime::createFromFormat('Y-m-d', $record['start_date']);
              echo $dt ? $dt->format('m/d/Y') : htmlspecialchars($record['start_date']);
            } else {
              echo '';
            }
          ?>
        ...
        <td>
          <?php
            if (!empty($record['end_date'])) {
              $dt = DateTime::createFromFormat('Y-m-d', $record['end_date']);
              echo $dt ? $dt->format('m/d/Y') : htmlspecialchars($record['end_date']);
            } else {
              echo '';
            }
          ?>
  ...
  <?php if (!empty($jo_work_experience)): ?>
      <?php foreach ($jo_work_experience as $exp): ?>
          <tr>
              <td>
                <?php
                  if (!empty($exp['date_from'])) {
                    $dt = DateTime::createFromFormat('Y-m-d', $exp['date_from']);
                    echo $dt ? $dt->format('m/d/Y') : htmlspecialchars($exp['date_from']);
                  } else {
                    echo '';
                  }
                ?>
              ...
              <td>
                <?php
                  if (!empty($exp['date_to'])) {
                    $dt = DateTime::createFromFormat('Y-m-d', $exp['date_to']);
                    echo $dt ? $dt->format('m/d/Y') : htmlspecialchars($exp['date_to']);
                  } else {
                    echo '';
                  }
                ?>
  ...
  <?php if (!empty($contract_service_records)): ?>
    <?php foreach ($contract_service_records as $record): ?>
      <tr>
        <td>
          <?php
            if (!empty($record['contractStart'])) {
              $dt = DateTime::createFromFormat('Y-m-d', $record['contractStart']);
              echo $dt ? $dt->format('m/d/Y') : htmlspecialchars($record['contractStart']);
            } else {
              echo '';
            }
          ?>
        ...
        <td>
          <?php
            if (!empty($record['contractEnd'])) {
              $dt = DateTime::createFromFormat('Y-m-d', $record['contractEnd']);
              echo $dt ? $dt->format('m/d/Y') : htmlspecialchars($record['contractEnd']);
            } else {
              echo '';
            }
          ?>
...
<!-- Left: Prepared by section -->
<div style="margin-left: 16px; text-align: left; font-size: 17px;">
  <div style="font-weight: bold; font-size: 18px;">Prepared by:</div>
  <br><br><br> <!-- Space for signature -->
  <div style="margin-left: 10px; text-align: left;">
    <div style="text-decoration: underline; font-size: 18px;"><strong>John Ryan C. Dela Cruz, Ph.D.</div></strong>
    <div style="text-align: center; font-size: 14px;">Supervising Administrative Officer</div>
  </div>
</div>
  </div>

<!-- Right: Date and source -->
