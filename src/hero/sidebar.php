...
if (isset($_SESSION['user_id'])) {
    ...
    $stmt->bind_param("i", $user_id);  // "i" means it's an integer parameter
    $stmt->execute();
    ...
    if ($user) {
        $user_name = $user['email'];  // Assuming 'email' is used as the user name
    } else {
        $user_name = 'Guest';  // Default name in case the user doesn't exist
    }
} else {
    $user_name = 'Guest';  // Default name if the session is not set
}
?>
...
<div class="sidebar" id="sidebar">
    ...
    <div class="d-flex align-items-center justify-content-start mb-3 ps-3">
        ...
        <p class="m-0 ms-2"><?php echo $user['email']; ?></p>
    </div>
